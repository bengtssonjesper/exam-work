...
export default function GraphicView() {
    ...
    function handleMouseMove(e){
        if(moveHandlerEnabled){
            ...
            seats.forEach(seat=>{
                if(
                    (hoverX>=seat.x && hoverX<=(seat.x+60))&&
                    (hoverY>=seat.y && hoverY <=(seat.y+40))){
                        // setPointer(false)
                        isPointer=true
                        //Här vill vi veta om den klickade platsen var röd eller grön, hur får vi reda på det?
                    }
    ...
    return (
        <div>
            <Form onSubmit={handleSubmit} className="mb-3">
                ...
                <Button className="mt-2 mb-2" type="submit">Update</Button>
            </Form>
            <canvas 
            style={{visibility:showCanvas?'visible':'hidden', cursor:pointer?'pointer':'default'}}
            id="app-canvas"
